...
def parse_args() -> StressConfig:
    parser = argparse.ArgumentParser(description="Adaptive BadNet patch stress test against a fixed purifier.")
    ...
    parser.add_argument("--classifier_ckpt", default="Imagenette2_pretrain/BadNet/Res_34_256/ckpt.pth")
    ...
    parser.add_argument("--attack_seed", type=int, default=-1, help="Seed for original BadNet attack generation.")
    ...
    parser.add_argument(
        "--adaptive_init",
        choices=["original_badnet", "random"],
        default="original_badnet",
...
def patch_slices(config: StressConfig) -> tuple[slice, slice]:
    if config.img_size <= 128:
        return slice(-6, -3), slice(-6, -3)
    return slice(-12, -3), slice(-12, -3)


def patch_mask(config: StressConfig, device: torch.device) -> torch.Tensor:
    mask = torch.zeros((3, config.img_size, config.img_size), dtype=torch.float32, device=device)
    row_slice, col_slice = patch_slices(config)
...
def initial_pattern(config: StressConfig, device: torch.device) -> torch.Tensor:
    ...
    generator.manual_seed(config.optim_seed)
    pattern = torch.zeros((3, config.img_size, config.img_size), dtype=torch.float32, device=device)
    row_slice, col_slice = patch_slices(config)
    patch_size = 3 if config.img_size <= 128 else 9
    pattern[:, row_slice, col_slice] = torch.rand((3, patch_size, patch_size), generator=generator, device=device)
    return pattern


def original_badnet_pattern(config: StressConfig, device: torch.device) -> torch.Tensor:
    """Recreate the BackdoorBox BadNet patch used by attack.backdoorattack.

    The project builds a 3-channel uint8 full-image pattern with nonzero pixels
    only in the bottom-right patch. The equivalent tensor-space pattern is
    pattern / 255.
    """
    generator = torch.Generator(device="cpu")
    generator.manual_seed(config.attack_seed)
    pattern = torch.zeros((3, config.img_size, config.img_size), dtype=torch.uint8)
    row_slice, col_slice = patch_slices(config)
    patch_size = 3 if config.img_size <= 128 else 9
    patch = (torch.randn((3, patch_size, patch_size), generator=generator) * 255).to(torch.uint8)
    pattern[:, row_slice, col_slice] = patch
...
def adaptive_initial_pattern(config: StressConfig, device: torch.device) -> torch.Tensor:
    if config.adaptive_init == "original_badnet":
        return original_badnet_pattern(config, device)
...
def pattern_to_badnet_uint8(pattern: torch.Tensor) -> torch.Tensor:
    return pattern.detach().cpu().mul(255.0).round().clamp(0, 255).to(torch.uint8)


def badnet_weight(config: StressConfig) -> torch.Tensor:
    return patch_mask(config, torch.device("cpu"))


def build_adaptive_badnet_poisoned_test(
    config: StressConfig,
    ...
    pattern_uint8 = pattern_to_badnet_uint8(adaptive_pattern * patch_mask(config, adaptive_pattern.device))
    weight = badnet_weight(config)
    badnet_cls = type(original_backdoor_instance)
    adaptive_instance = badnet_cls(
...
def apply_badnet_trigger(images: torch.Tensor, pattern: torch.Tensor, config: StressConfig) -> torch.Tensor:
    mask = patch_mask(config, pattern.device)
    return torch.clamp((1.0 - mask.unsqueeze(0)) * images + mask.unsqueeze(0) * pattern.unsqueeze(0), 0.0, 1.0)
...
def optimize_pattern(
    ...
    for step in range(1, config.steps + 1):
        ...
        triggered = apply_badnet_trigger(images, constrained_pattern, config)
...
def evaluate_trigger_without_pipeline(
    ...
    for images, _ in loader:
        images = images.to(device)
        triggered = apply_badnet_trigger(images, pattern.to(device), config)
...
def add_canonical_baseline(results: dict[str, Any]) -> None:
    ...
    elif "main_original_asr_after_pipeline" in results:
        source = "generated_main_pipeline_output"
        canonical_rate = results["main_original_asr_after_pipeline"]
        canonical_confidence = results["main_original_mean_target_confidence"]
        comparable_to_main_asr = True
    else:
...
def write_results(config: StressConfig, results: dict[str, Any], adaptive_pattern: torch.Tensor) -> Path:
    ...
    run_id = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    result_path = output_dir / f"{run_id}_{config.dataset}_BadNet_PABR.json"
...
def main() -> int:
    ...
    original_pattern = original_badnet_pattern(config, device)
    ...
    if config.final_eval_via_main_pipeline:
        ...
        seed_everything(config.attack_seed)
        ...
        adaptive_poisoned_test = build_adaptive_badnet_poisoned_test(
            config,
