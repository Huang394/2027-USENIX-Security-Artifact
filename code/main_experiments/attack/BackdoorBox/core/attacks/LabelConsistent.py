...
class PoisonedDatasetFolder(DatasetFolder):
    ...
    def __getitem__(self, index):
        ...
        return sample, {'label_orig': orig_target, 'label_pois': target}
...
class PoisonedMNIST(MNIST):
    ...
    def __getitem__(self, index):
        ...
        return img, {'label_orig': orig_target, 'label_pois': target}
        ...
        def _generate_adv_dataset(dataset, adv_model, adv_dataset_dir, adv_transform, eps, alpha, steps, y_target, poisoned_rate):
            def _dataset_targets(dataset):
                if hasattr(dataset, "targets"):
                    ...
                    raise AttributeError("LabelConsistent requires dataset.targets or dataset.samples to build poisoned_set.")
                if dataset.target_transform is not None:
                    targets = [dataset.target_transform(target) for target in targets]
            ...
            y_target_index_list = np.atleast_1d(y_target_index_list)
            ...
            for batch in tqdm(data_loader):
                ...
                original_imgs = torch.round(batch_img * 255).to(dtype=torch.uint8).permute(0, 2, 3, 1).detach().cpu().numpy()
                perturbed_imgs = img.permute(0, 2, 3, 1).detach().cpu().numpy()
                batch_targets = batch_label.detach().cpu().numpy()

                for batch_offset, target in enumerate(batch_targets):
                    original_img = original_imgs[batch_offset]
                    perturbed_img = perturbed_imgs[batch_offset]
                    target = int(target)
                    cv2.imwrite(
                        osp.join(adv_dataset_dir, 'whole_adv_dataset', str(target).zfill(2), str(sample_index).zfill(8) + '.png'),
                        perturbed_img,
                    )

                    if sample_index in poisoned_set:
                        target_img = perturbed_img
                    else:
                        target_img = original_img
                    cv2.imwrite(
                        osp.join(adv_dataset_dir, 'target_adv_dataset', str(target).zfill(2), str(sample_index).zfill(8) + '.png'),
                        target_img,
                    )
                    sample_index += 1
