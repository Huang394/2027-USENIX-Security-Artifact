...
from .base import Base, accuracy
...
def _to_uint8_image(image, normalize=False):
    if normalize:
        image_min = image.min()
        image_max = image.max()
...
class ISSBA(Base):
    ...
    def _encode_batch_with_secret(self, encoder, images, secret):
        batch_secret = secret.unsqueeze(0).repeat(images.size(0), 1)
        with torch.no_grad():
            residual = encoder([batch_secret, images])
            encoded_image = images + residual
            return encoded_image.clamp(0, 1)

    def _test_poisoned_online(self, dataset, device, batch_size=16, num_workers=8, model=None):
        if model is None:
            model = self.model
        secret = torch.FloatTensor(
            np.random.binomial(1, .5, self.encoder_schedule['secret_size']).tolist()
        ).to(device)
        ...
        with torch.no_grad():
            ...
            for batch_img, _ in test_loader:
                ...
                batch_img = self._encode_batch_with_secret(encoder, batch_img, secret)
                if self.normalizer:
                    batch_img = self.normalizer(batch_img)
                predict_digits.append(model(batch_img).cpu())
                labels.append(torch.full((batch_img.size(0),), self.y_target, dtype=torch.long))
    ...
    def train_encoder_decoder(self, train_only=False):
        ...
        if lpips is None:
            raise ImportError("ISSBA encoder training requires the optional 'lpips' package.")
    ...
    def train(self, schedule=None):
        if torch.are_deterministic_algorithms_enabled():
            print("ISSBA uses grid_sample backward; disabling deterministic algorithms for this attack.")
        ...
        if self.current_schedule.get('pretrain'):
        ...
        self.work_dir = osp.join(
            ...
            'Res_' + str(self.current_schedule['layers']) + '_' + str(self.current_schedule['img_size']),
        )
        ...
        poisoned_indices = sorted(self.poisoned_set)
        clean_indices = [idx for idx in range(len(trainset)) if idx not in self.poisoned_set]
        cln_train_dataset = Subset(trainset, clean_indices)
        bd_train_dataset = Subset(trainset, poisoned_indices)
        cln_test_dl = testset

        bd_bs = int(self.current_schedule['batch_size'] * self.poisoned_rate)
        bd_bs = max(1, bd_bs)
        cln_bs = max(1, int(self.current_schedule['batch_size'] - bd_bs))
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            loss_list = []
            for (inputs, targets), (inputs_trigger, _targets_trigger) in zip(cln_train_dl, bd_train_dl):
                inputs = inputs.to(device)
                inputs_trigger = inputs_trigger.to(device)
                inputs_trigger = self._encode_batch_with_secret(encoder, inputs_trigger, secret)
                targets_trigger = torch.full(
                    (inputs_trigger.size(0),),
                    self.y_target,
                    dtype=torch.long,
                    device=device,
                )
                inputs = torch.cat((inputs, inputs_trigger), 0)
                targets = torch.cat((targets.to(device), targets_trigger), 0)
                if self.normalizer:
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                predict_digits, labels = self._test_poisoned_online(
                    testset,
                    device,
    ...
    def get_img(self, path=None):
        ...
        for _, (image_input, secret_input) in enumerate(train_dl):
            ...
            image_input = _to_uint8_image(image_input.detach().cpu().numpy().transpose(0, 2, 3, 1)[0])
            encoded_image = _to_uint8_image(encoded_image.detach().cpu().numpy().transpose(0, 2, 3, 1)[0])
            residual = _to_uint8_image(residual.detach().cpu().numpy().transpose(0, 2, 3, 1)[0], normalize=True)
