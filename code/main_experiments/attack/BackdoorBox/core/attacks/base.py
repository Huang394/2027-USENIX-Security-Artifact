...
from torchvision.datasets import DatasetFolder, MNIST, CIFAR10, CelebA  # noqa: F401
from attack.originalimagenet import Origdataset
from preprocess.purification import FolderBackedDataset, Purify, SplitCLeanDataset, SplitDataset, nonSplitDataset  # noqa: F401
from preprocess.guided_diffusion.purify import (  # noqa: F401
    SplitCLeanDataset as ZipSplitCLeanDataset,
    SplitDataset as ZipSplitDataset,
    nonSplitDataset as ZipNonSplitDataset,
)
import torchvision.utils as tvu  # noqa: F401
...
def _load_state_dict_compatible(model, ckpt_model_path):
...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        if self.current_schedule['pretrain'] is True:
            ...
            _load_state_dict_compatible(self.model, ckpt_model_path)
            self.model.eval()
            print("=================================================    Loaded    ==================================================")
    ...
    def _asrtest(self, dataset, device, batch_size=1, num_workers=8, model=None, label_type='label_pois'):
        ...
        with torch.no_grad():
            ...
            for batch in test_loader:
                ...
                batch_img = model(batch_img)
                batch_img = batch_img.cpu()
                keep_mask = batch_label['label_orig'] != batch_label['label_pois']
                batch_img = batch_img[keep_mask]
                batch_target = batch_label[label_type][keep_mask]
                if batch_target.numel() == 0:
                    continue
                predict_digits.append(batch_img)
            ...
            if len(predict_digits) == 0:
                raise RuntimeError("No poisoned test samples remain after filtering label_orig == label_pois.")
            predict_digits = torch.cat(predict_digits, dim=0)
            labels = torch.cat(labels, dim=0)
            return predict_digits, labels
