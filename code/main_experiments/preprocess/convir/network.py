...
class BasicConv(nn.Module):
    def __init__(self, in_channel: int, out_channel: int, kernel_size: int, stride: int, bias: bool = True, norm: bool = False, relu: bool = True, transpose: bool = False) -> None:
        super().__init__()
        if bias and norm:
            bias = False
...
class ResBlock(nn.Module):
    def __init__(self, in_channel: int, out_channel: int, filter: bool = False) -> None:
        super().__init__()
...
class DeepPoolLayer(nn.Module):
    def __init__(self, k: int, k_out: int) -> None:
        super().__init__()
        self.pools_sizes = [8, 4, 2]
        dilation = [7, 9, 11]
        pools, convs, dynas = [], [], []
        for idx, size in enumerate(self.pools_sizes):
            pools.append(nn.AvgPool2d(kernel_size=size, stride=size))
            convs.append(nn.Conv2d(k, k, 3, 1, 1, bias=False))
            dynas.append(MultiShapeKernel(dim=k, kernel_size=3, dilation=dilation[idx]))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_size = x.size()
        resl = x
        y_up: torch.Tensor | None = None
        for i in range(len(self.pools_sizes)):
            if i == 0:
                y = self.dynas[i](self.convs[i](self.pools[i](x)))
            else:
                assert y_up is not None
...
class dynamic_filter(nn.Module):
    def __init__(self, inchannels: int, kernel_size: int = 3, dilation: int = 1, stride: int = 1, group: int = 8) -> None:
        super().__init__()
        self.stride = stride
...
class cubic_attention(nn.Module):
    def __init__(self, dim: int, group: int, dilation: int, kernel: int) -> None:
        super().__init__()
...
class spatial_strip_att(nn.Module):
    def __init__(self, dim: int, kernel: int = 3, dilation: int = 1, group: int = 2, H: bool = True) -> None:
        super().__init__()
        self.k = kernel
...
class MultiShapeKernel(nn.Module):
    def __init__(self, dim: int, kernel_size: int = 3, dilation: int = 1, group: int = 8) -> None:
        super().__init__()
...
def build_net(version: str) -> ConvIR:
